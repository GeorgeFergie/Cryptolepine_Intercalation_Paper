...
def get_nwchem_calculator(f_charge=-3,
    ...
    if f_solv.upper() == "WATER":
        ...
        tmp["cosmo"] = dict(solvent='water')#(do_cosmo_smd=True, solvent='water')
...
calc = get_nwchem_calculator(f_charge=-3,
                             f_xc_functional="BECKE88 LYP", 
                             f_multiplicity=1,
                             f_basis_set="6-31G",
                             f_disp="D3",
                             f_solv="WATER")
# load the structure
atoms = ase.io.read("CC_BB_Opti.xyz@-1")
# set the calculator
atoms.calc = calc
# optimise the structure
Optimise = ase.optimize.BFGS(atoms, trajectory="CC_BB_Opti.traj", logfile="CC_BB_Opti.log")
